...
Ported 2026-09-13 from ../ai-eng-bootcamp.vera/scripts/refresh_openai_pricing.py
(only the import path changed). That project's *first* pricing refresh
attempt (table-walk + regex fallback) was deprecated as a documented
anti-pattern after live testing found it substring-collided on model names
(`gpt-4o` matching inside `gpt-4o-mini`) and confused cached-input for output
whenever 3+ price columns appeared in sequence — see that project's
pricing-scraper-design-prv.md for the full writeup. This script is the
*replacement* that avoids both failure modes structurally, not by patching
the old heuristics:
...
Deliberately out of scope (skipped, not guessed):
  ...
  - Batch and Fast mode tiers — out of scope, matching the source project's
    standing constraint: strictly OpenAI's own page, Standard + Flex only.
...
is never invoked by main.py at request time.
...
def fetch_page(url: str) -> str:
    req = urllib.request.Request(url, headers={"User-Agent": "week-1v2-pricing-refresh/1.0"})
...
def latest_for_model_and_tier(model: str, tier: str, pricing: ModelPricing | None) -> PricingRecord | None:
    """Like pricing_config.latest_pricing_for, but tier-aware. That helper
    ignores service_tier entirely (it was written for main.py's per-request
    single-tier lookup), so reusing it here would compare a model's Standard
    price against whichever tier happens to have the latest effective_from
