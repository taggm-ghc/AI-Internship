...
This project's real price-collection pipeline is deliberately no-human,
no-browser: scripts/refresh_openai_pricing.py parses the page's own
embedded JSON directly. This script is NOT part of that pipeline — it is
NOT a scraping/parsing script, does not write to config/model-pricing.json,
and is not meant to be run routinely. Its only legitimate use is an
occasional, manual audit of whether refresh_openai_pricing.py's structural
assumptions (tier labels, table shape) still hold on the live page — never
a required step, and never invoked by main.py or any deployed code path.

Ported 2026-09-14 from
../ai-eng-bootcamp.vera/scripts/capture_openai_pricing.py (only the import
path changed; the mechanism below was verified live in that project before
being written, not assumed — see its pricing-scraper-design-prv.md,
"Capture-script flow and pseudocode" section, for the full design writeup,
including two false starts recorded there so they aren't repeated here):
the Flagship hero table and the full "additional models" list share one
segmented control and one scoped container.

Requires Playwright + Chromium — see requirements-dev.txt, not
requirements.txt.
...
# Scope matches scripts/refresh_openai_pricing.py's standing constraint:
# strictly OpenAI's own page, Standard + Flex only — Batch/Fast mode not
# requested.
TIERS = ["standard", "flex"]
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Un-collapse the full model table. The Flagship-family rows and
            # the full non-flagship list are the same table, collapsed to a
            # handful of rows by default; this button (not a separate
            # section/control) reveals the rest.
